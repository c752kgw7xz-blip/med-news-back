...
# Fonds PISTE complémentaires pertinents pour les pros de santé
EXTRA_FONDS = {
    "KALI": "piste_kali",      # Conventions collectives (UNCAM)
    "LEGI": "piste_legi",      # Codes législatifs
    "CIRC": "piste_circ",      # Circulaires ministérielles
}


@router.post("/collect-extra")
def collect_extra_fonds(
    ...
    """
    Collecte des fonds PISTE complémentaires (KALI, LEGI, CIRC).
    Insère dans candidates avec déduplication.
    """
    _require_admin(request)

    fond = fond.upper()
    if fond not in EXTRA_FONDS:
        raise HTTPException(status_code=400, detail=f"Fonds inconnu: {fond}. Valides: {list(EXTRA_FONDS.keys())}")

    source = EXTRA_FONDS[fond]
    today = date.today()
    start = today - timedelta(days=days)

    seen = inserted = deduped = pages_fetched = 0
    stop_reason: str | None = None

    insert_sql = """
    INSERT INTO candidates (
      source, official_url, official_date, title_raw,
      content_raw, raw_json, raw_sha256, dedupe_key, status
    )
    VALUES (
      %(source)s, %(official_url)s, %(official_date)s, %(title_raw)s,
      %(content_raw)s, %(raw_json)s, %(raw_sha256)s, %(dedupe_key)s, 'NEW'
    )
    ON CONFLICT (dedupe_key) DO NOTHING;
    """
        ...
        with conn.cursor() as cur:
            for page_number in range(1, max_pages + 1):
                ...
                for it in items:
                    ...
                    title = t0.get("title") or it.get("titre") or "(no title)"
                    ...
                    official_url = f"https://www.legifrance.gouv.fr/search?fond={fond}&id={text_id}" if text_id else ""
                    dedupe_key = _sha256_hex(f"{source}|{text_id}".encode("utf-8"))
                    raw_bytes = _json_canonical_bytes(it)
