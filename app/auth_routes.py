...
def set_auth_cookies(resp: Response, refresh_token: str, csrf_token: str):
    # Refresh token: HttpOnly, Secure — path="/" pour que le navigateur l'envoie à /auth/refresh et /auth/logout
    resp.set_cookie(
        key=REFRESH_COOKIE,
        value=refresh_token,
        httponly=True,
    ...
    # CSRF token: readable by JS (double-submit) — path="/" indispensable pour que
    # document.cookie le voie depuis /portal, /review, etc.
    resp.set_cookie(
        key=CSRF_COOKIE,
        value=csrf_token,
        httponly=False,
...
def refresh(request: Request, response: Response):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            # find valid token — FOR UPDATE prevents concurrent refresh with the same token
            cur.execute(
                """
                SELECT id, user_id
                FROM refresh_tokens
